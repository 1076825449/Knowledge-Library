...
1. **最薄弱模块优先补题**：CLEAR(28) / TAX(25) / CIT(24) / FEE(26) / VAT(28) — 这些模块问题数最少
2. **问题类型补齐**：type_compare(2条) / type_time(4条) / type_why(4条) 类型过少
3. **scope_local 补强**：当前仅 2 条地方口径内容，需扩展至 30+ 条
4. **高频问题群补强**：零申报、发票红字、欠税非正常户、注销清算等政策依据补强
5. **关联关系补强**：为问题补关联关系，提升知识库路径式使用体验
