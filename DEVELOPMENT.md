...
│   ├── content/            # 内容脚本测试
│   ├── database/           # 数据库结构测试
│   └── conftest.py         # pytest 全局配置（werkzeug 兼容 patch）
├── requirements.txt
└── .gitignore
```
...
| `tag_dict` | 标签字典 | id, tag_code, tag_name, tag_category |
...
| `question_update_log` | 更新记录 | question_id, version_no, update_type, change_summary |
| `local_rule_note` | 地方口径 | id, question_id, region_name, authority_name, local_content |
...
| `answer_certainty` | certain_clear, certain_conditional, certain_dispute, certain_practice |
| `scope_level` | scope_national, scope_local, scope_mixed |
| `question_type` | type_whether, type_how, type_define, type_risk, type_time, type_what, type_why |
| `relation_type` | related, next_step, prerequisite, similar, see_also |
| `support_type` | support_direct, support_aux, support_definition, support_procedure, support_risk, support_local |
| `update_type` | update_new, update_revise, update_policy, update_boundary, update_local, update_status |
...
# 4. 提交（.db 文件和 .json 导入源文件需单独 commit）
git add database/db/tax_knowledge.db
git add data/imports/questions_batch*.json
git add scripts/ backend/ tests/
git commit -m "feat: ..."
...
```bash
# 1. 写 JSON 批导入文件（参考 data/imports/questions_batch*.json）
