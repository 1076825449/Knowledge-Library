...
| Phase 5 | 检索、筛选与关联问题增强 | ⚠️ 已有基础能力，仍需统一口径与继续打磨 |
| Phase 6 | 内容录入与维护便利化 | ⚠️ 已有录入原型，未达到长期维护闭环 |
| Phase 7 | 地方口径、更新机制与专业增强 | ⚠️ 已有结构和部分数据，远未完成 |
| Phase 8 | AI 检索与高级能力预留 | ⚠️ 仅完成结构预留，尚未形成稳定 contract |

> 当前补充说明：部署闭环基础版已完成，包括 Gunicorn 生产入口、Dockerfile、Procfile、环境变量口径和部署文档；但正式上线仍需域名、HTTPS、反向代理和运行环境托管决策。

> 自本次更新起，本文件保留“任务拆解”作用，不再把后续阶段简单标记为“已完成”。是否真正进入下一阶段，以 `ROADMAP.md` 的验收标准和数据库、质量报告的事实状态为准。
...
本阶段已经有基础实现，但还没有达到“稳定、统一、可长期扩展”的状态。
...
- 筛选口径和枚举展示口径不完全一致
- 关联问题虽已有大量数据，但仍存在缺口
- 地区筛选仍偏向 `scope_level`，未充分支持真实地方口径表达
...
#### 8.3 关联问题增强
- [x] 在详情页展示关联问题列表
- [x] 支持不同关联类型（`related`/`next_step`/`prerequisite`/`similar`/`see_also`，共5种；详情页展示类型标签）
- [x] 优化关联问题排序
...
本阶段已有新建页和编辑页，但只能视为“录入原型已存在”，不能视为已经完成。

当前仍需继续处理：

- 表单枚举和数据库实际值需进一步统一
- 政策依据关联数量和录入灵活性仍偏弱
- 更新记录、关联问题、地方口径的维护入口还不够完整
- 还缺真正稳定的草稿、待复核、启用维护流程
...
本阶段目前只能算“已有导出与结构预留”，还没有达到真正可稳定复用的 AI 检索底座。

当前仍需继续处理：

- 明确稳定的导出字段 contract
- 明确切块和引用规则
- 保证 AI 输出可稳定回指 `question_code` 与政策依据
- 在未统一 contract 前，不应过早接入更重的智能层
...
### 高优先级问题群（按短板优先级排序，截至 2026-04-20）
- [x] 企业所得税基础（CIT 模块 38 条，已达标）
- [x] 成本费用与凭证取得（FEE 模块 36 条）
- [x] 增值税深化（VAT 模块 39 条）
- [x] 清税注销（CLEAR 模块 44 条，已达标）
- [x] 税务综合（TAX 模块 47 条，已达标）
- [x] 地方口径补强（scope_local 10 条 ✅）
- [x] type_why 类型问题（12 条 ✅，较目标4条翻三倍）
- [x] type_compare 类型问题（16 条 ✅）
- [ ] 个税代扣代缴深度问题（IIT 模块 38 条，可继续扩容）
- [ ] 设立后首次涉税事项（REG/SET 模块深度）
- [ ] 无收入 / 未经营 / 零申报专题
- [ ] 发票领用与开具深度问题（INV 模块 47 条）
- [ ] 社保费深度问题（SSF 模块 47 条）
- [ ] 变更登记后税务处理（CHG 分散各模块，已覆盖）
- [ ] 风险预警与异常处理深度（RISK 模块 59 条，可继续扩容）
- [ ] 注销清税深度问题（CLEAR 模块 44 条）
- [ ] type_time 类型问题（当前 5 条，可小幅补强）
